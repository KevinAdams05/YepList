...
namespace ToDoList.Api.Controllers
{
    ...
    public class TodoListsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            var dtos = lists.Select(l => MapToDto(l));
            return Ok(dtos);
        }
        ...
        {
            var list = await listRepository.InsertAsync(request.Name, request.SortOrder);
        ...
        {
            var list = await listRepository.UpdateAsync(id, request.Name, request.SortOrder);
            if (list == null)
            {
                return NotFound();
            }
        ...
        {
            var deleted = await listRepository.DeleteAsync(id);
            if (!deleted)
            {
                return NotFound();
            }
